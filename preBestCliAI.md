...
16. [Automatyczna optymalizacja zadań — CLAUDE.md jako optimization layer](#16-automatyczna-optymalizacja-zadań--claudemd-jako-optimization-layer)
17. [Źródła](#17-źródła)
...
## 16. Automatyczna optymalizacja zadań — CLAUDE.md jako optimization layer

### Koncepcja "optimization by default"

Tradycyjne podejście do optymalizacji kodu jest **opt-in** — programista musi pamiętać o stosowaniu dobrych praktyk. Z agentem AI CLI można to odwrócić: **wbudować optymalizację w środowisko**, tak aby KAŻDE zadanie było automatycznie optymalizowane.

**Mechanizm**: CLAUDE.md (lub AGENTS.md) działa jako **system prompt** dla każdego zapytania w Claude Code CLI. Wpisanie reguł optymalizacji do tego pliku oznacza, że agent stosuje je **zawsze**, bez dodatkowego wysiłku użytkownika.

**Dlaczego to działa**:
- Agent czyta CLAUDE.md przy KAŻDYM zapytaniu — reguły są nierozerwalnie częścią każdej sesji
- Brak "ludzkiego zapominania" — agent NIE pominie reguły przez zmęczenie lub pośpiech
- Kumulatywny efekt — tysiące drobnych optymalizacji per projekt przekładają się na jakość całego codebase

**Dlaczego to NIE jest to samo co "pisz czysty kod"**:
Ogólnikowe instrukcje ("pisz optymalny kod") nie działają — model je ignoruje. Skuteczne reguły są **konkretne i weryfikowalne**:

| Nieskuteczne (ignorowane) | Skuteczne (egzekwowane) |
|---------------------------|-------------------------|
| "Pisz optymalny kod" | "Stosuj early returns zamiast nestowania >2 poziomów" |
| "Bądź wydajny" | "Batchuj zapytania DB — max 1 query per collection" |
| "Unikaj błędów" | "Każda publiczna funkcja: waliduj input, zwróć Result/Error" |

### Gotowy szablon CLAUDE.md wymuszający optymalizację

Poniższy szablon jest gotowy do skopiowania do `CLAUDE.md` dowolnego projektu:

```markdown
# Zasady optymalizacji kodu

## Przed napisaniem kodu
- ZANIM napiszesz pierwszą linię: przeanalizuj wymagania i zaplanuj architekturę
- Rozważ edge cases i potencjalne problemy wydajnościowe Z GÓRY
- Wybierz najbardziej efektywny algorytm dla danego problemu
- Jeśli zadanie dotyka >3 plików: użyj Plan Mode, nie implementuj od razu

## Podczas pisania kodu
- Stosuj early returns / guard clauses zamiast głębokiego nestowania (max 2 poziomy if)
- Preferuj const/readonly/frozen — mutowalne zmienne tylko gdy konieczne
- Używaj odpowiednich struktur danych:
  - Map/Hash zamiast Object/Array dla częstych lookup-ów
  - Set zamiast Array dla unikalności
  - Struct/namedtuple zamiast loose Hash dla znanych kształtów danych
- Minimalizuj operacje I/O — batchuj zapytania do bazy/API
- Unikaj N+1 queries — załaduj powiązane dane w jednym zapytaniu
- Nie alokuj w pętlach — przenieś alokacje przed pętlę

## Po napisaniu kodu
- Przejrzyj pod kątem: zbędnych powtórzeń, nieużywanych zmiennych, niepotrzebnych zależności
- Oceń złożoność obliczeniową (Big O) kluczowych operacji
- Zaproponuj testy dla krytycznych ścieżek i edge cases
- Sprawdź czy nie wprowadzono regresji wydajnościowych

## Ogólne zasady
- DRY — nie powtarzaj kodu, ale NIE abstrahuj przedwcześnie (3 powtórzenia = czas na abstrakcję)
- KISS — prostsze rozwiązanie jest lepsze, jeśli spełnia wymagania
- YAGNI — nie buduj na przyszłość, buduj na teraz
- Każda funkcja robi jedną rzecz dobrze
- Nazwy zmiennych/funkcji jasno opisują przeznaczenie (bez skrótów)
```

### Hierarchia optymalizacji (3 poziomy)

CLAUDE.md obsługuje hierarchiczne ładowanie — wykorzystaj to do warstw optymalizacji:

```
~/.claude/CLAUDE.md              → globalne reguły (wszystkie projekty)
  ├── Ogólne zasady kodowania
  ├── Preferowany styl (early returns, guard clauses)
  └── Bezpieczeństwo (walidacja inputu, parametryzowane SQL)

./CLAUDE.md                      → per projekt
  ├── Stack-specific (Rails: avoid N+1, React: useMemo)
  ├── Architektura projektu
  └── Workflow (testy, deploy, review)

./src/api/CLAUDE.md              → per moduł/katalog
  ├── API conventions (pagination, error format)
  └── Performance budgets (max response time)
```

**Warstwy są addytywne** — Claude czyta WSZYSTKIE pliki w hierarchii. Głębsze pliki mają priorytet przy konfliktach.

**Praktyczny przykład**:
- Globalnie: "Stosuj early returns"
- Projekt Rails: "Unikaj N+1 — używaj `includes()` / `preload()`"
- Katalog API: "Każdy endpoint: max 3 queries, paginacja domyślnie 25"

### Reguły technologicznie-specyficzne (`.claude/rules/`)

Dla reguł zależnych od typu pliku, użyj `.claude/rules/` z glob scoping:

```markdown
# .claude/rules/ruby-optimization.md
---
globs: ["**/*.rb", "**/*.rake"]
---
- Użyj `find_each` zamiast `each` dla dużych kolekcji ActiveRecord
- Preferuj `pluck(:id)` zamiast `map(&:id)` dla kolumn
- Unikaj `where.not` na dużych tabelach — rozważ indeks
- Stosuj `bulk_insert` zamiast pętli `create` dla >10 rekordów
```

```markdown
# .claude/rules/typescript-optimization.md
---
globs: ["**/*.ts", "**/*.tsx"]
---
- Strict TypeScript — zero `any`, zero `as` castów bez uzasadnienia
- React: useMemo/useCallback dla kosztownych obliczeń i stabilnych referencji
- Preferuj `Map<K,V>` nad `Record<string, V>` dla dynamicznych kluczy
- Async/await zamiast .then() chains — czytelność + error handling
```

```markdown
# .claude/rules/python-optimization.md
---
globs: ["**/*.py"]
---
- Generatory zamiast list comprehensions dla dużych zbiorów danych
- `__slots__` w klasach z wieloma instancjami
- `functools.lru_cache` dla powtarzalnych obliczeń
- Type hints obowiązkowe w publicznych interfejsach
```

### Wzorce kodu wymuszane przez CLAUDE.md

#### Early returns / Guard clauses

**Zły wzorzec** (głębokie nestowanie):
```javascript
...
**Dobry wzorzec** (guard clauses):
```javascript
function processUser(user) {
  if (!user) return "Brak użytkownika";
  if (!user.isActive) return "Nieaktywny";
  if (!user.hasPermission) return "Brak uprawnień";
  if (user.age < 18) return "Za młody";

  return doSomething(user);
}
```

**Dlaczego**: Czytanie liniowe od góry do dołu. Mniejsze obciążenie poznawcze. Łatwiejsze utrzymanie — nowy warunek = jedna linijka. Mniej błędów z pomieszanych bloków if/else.
...
```javascript
// Źle: O(n) lookup w Array
const found = items.find(i => i.id === targetId);

// Dobrze: O(1) lookup w Map
const itemMap = new Map(items.map(i => [i.id, i]));
const found = itemMap.get(targetId);
```

### Weryfikacja automatyczna — Hooks jako gwarancja

CLAUDE.md to **guidance** (advisory) — model może je zignorować. Hooks to **enforcement** (gwarancja):
...
**Rekomendacja**: Używaj CLAUDE.md dla reguł stylu i wzorców. Używaj Hooks dla weryfikacji, która MUSI się odbyć (linting, type-checking, testy).

| Mechanizm | Typ | Gwarancja | Użyj dla |
|-----------|-----|-----------|----------|
| CLAUDE.md | Guidance | Nie (advisory) | Wzorce, styl, architektura |
| `.claude/rules/` | Guidance | Nie (warunkowe) | Reguły per język/framework |
| Hooks | Enforcement | **Tak** | Linting, type-check, testy |
| Skills | Knowledge | Nie (on-demand) | Wiedza specjalistyczna |

### Podsumowanie: Optimization by default w 4 krokach

1. **Skopiuj szablon** z sekcji powyżej do `CLAUDE.md` swojego projektu
2. **Dodaj reguły per język** do `.claude/rules/` z odpowiednimi globami
3. **Skonfiguruj hooks** dla weryfikacji, która musi się odbyć (linting, testy)
4. **Iteruj** — gdy Claude powtarza błąd, dodaj regułę; gdy reguła ignorowana, przenieś do Hook

Efekt: każde zadanie w Claude Code CLI jest automatycznie optymalizowane, bez dodatkowego wysiłku.
