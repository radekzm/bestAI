...
# ============================================================
echo ""
echo "=== generate-rules.sh ==="
# ============================================================

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

OUTPUT=$(bash "$REPO_ROOT/tools/generate-rules.sh" --help 2>&1)
CODE=$?
assert_exit "generate-rules: --help -> exit 0" "0" "$CODE"
assert_contains "generate-rules: --help contains Usage" "$OUTPUT" "Usage:"

OUTPUT=$(node "$REPO_ROOT/bin/bestai.js" generate-rules --help 2>&1)
CODE=$?
assert_exit "bestai generate-rules --help -> exit 0" "0" "$CODE"
assert_contains "bestai generate-rules --help contains Usage" "$OUTPUT" "Usage:"

OUTPUT=$(node "$REPO_ROOT/bin/bestai.js" generate-rules . --format codex 2>&1)
CODE=$?
assert_exit "bestai generate-rules codex -> exit 0" "0" "$CODE"
assert_contains "bestai generate-rules codex header" "$OUTPUT" "AUTOGENERATED bestAI RULES FOR: CODEX"

OUTPUT=$(node "$REPO_ROOT/bin/bestai.js" --help 2>&1)
CODE=$?
assert_exit "bestai --help -> exit 0" "0" "$CODE"
assert_contains "bestai --help contains Usage" "$OUTPUT" "Usage:"
...
const bin = fs.readFileSync(path.join(root, 'bin', 'bestai.js'), 'utf8');
const required = ['swarm-lock', 'generate-rules', 'shared-context-merge'];
const missing = required.filter((cmd) => {
  const pattern = new RegExp(`['"]${cmd}['"]\\s*:`);
  return !pattern.test(bin);
});
if (missing.length > 0) {
  console.error(`Missing CLI mappings: ${missing.join(', ')}`);
  process.exit(1);
}
console.log('CLI mapping contract OK for required commands.');
NODE
2>&1)
CODE=$?
assert_exit "CLI contract: required mappings exist in bin/bestai.js" "0" "$CODE"
assert_contains "CLI contract: success message" "$OUTPUT" "CLI mapping contract OK"
